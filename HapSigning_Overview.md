...
Concise overview of the HAP signing scheme and how it compares to Android APK
signing. For byte-level layouts, see `HapStructure.md`.
...
- The signing block is inserted before the Central Directory.
...
- Signed content: encoded digest list
  - `version (2)`, `blockCount (1)`, and typically one `(algId, digest)` pair.
- Digest input covers:
  - ZIP entry contents (before Central Directory)
  - Central Directory
  - EOCD
  - Optional blocks (`profile` / `property` / `proof`)

### 1.3 Optional blocks
Optional blocks are sub-blocks inside the signing block and are included in
the main digest. They are loaded as raw file bytes (no parsing during signing).
...
If `signCode` is enabled, an additional PROPERTY block is inserted containing
the embedded codesign header + payload; it does not remove the original
PROPERTY block (if present).

### 1.4 Code sign (optional)
- When `signCode` is enabled, a codesign payload is generated and embedded as
  a PROPERTY sub-block value with an inner header (`type 0x30000001`).
...
The signing block tail uses a compatibility `version` (2 or 3) and
corresponding `magic`. This is separate from the main signature scheme version
(V1).
...
- Android v1 is JAR signing (per-entry manifest hashes); HAP main signature is
  a PKCS7 block over a digest list of ZIP segments.
- Android can include multiple schemes (v1/v2/v3) in one APK; the HAP toolchain
  currently uses a single main scheme type (V1 block ID), while the signing
  block has its own version (2/3) for compatibility.
- HAP adds a codesign layer (fs-verity + native libs) that is not part of
  Android's v1/v2/v3 schemes.

Notes:
- This comparison is functional/behavioral, not a formal spec mapping.
- See `HapStructure.md` for the exact byte-level layout in this repo.
